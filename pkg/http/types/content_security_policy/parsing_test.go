...
func findDirectiveByName(directives []DirectiveI, name string) DirectiveI {
...
func TestParseContentSecurityPolicy_Directives_TableDriven(t *testing.T) {
	...
	tests := []struct {
		name    string
		args    args
		check   func(t *testing.T, csp *ContentSecurityPolicy)
		wantErr bool
	}{
		{
			...
			check: func(t *testing.T, csp *ContentSecurityPolicy) {
				...
				sd := d.(*DefaultSrcDirective)
				if len(sd.Sources) != 1 {
					t.Fatalf("expected 1 source, got %d", len(sd.Sources))
				}
				if _, ok := sd.Sources[0].(*KeywordSource); !ok {
		...
		},
		{
			...
			check: func(t *testing.T, csp *ContentSecurityPolicy) {
				...
				sd := d.(*RequireTrustedTypesForDirective)
		...
		},
		{
			...
			check: func(t *testing.T, csp *ContentSecurityPolicy) {
				...
				if wd := d.(*WebrtcDirective); wd.Value != "allow" {
		...
		},
		{
			...
			check: func(t *testing.T, csp *ContentSecurityPolicy) {
				...
				sd := d.(*ObjectSrcDirective)
				if len(sd.Sources) != 1 {
					t.Fatalf("expected 1 source, got %d", len(sd.Sources))
				}
				if _, ok := sd.Sources[0].(*NoneSource); !ok {
